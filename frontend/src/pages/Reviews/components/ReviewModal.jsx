...
export default function ReviewModal({ review, onClose, onSaved, onUnauthorized, onConflict }) {
    ...
    async function submit(event) {
        ...
        try {
            ...
            if (error.status === 401) onUnauthorized();
                ...
                setErrors({ form: "Вы уже оставили отзыв. Закройте окно, чтобы открыть его для редактирования." });
                onConflict();
            } else setErrors({ form: "Не удалось сохранить отзыв. Ваш текст сохранён в форме — попробуйте снова." });
        } finally {
    ...
    return <Modal title={review ? "Редактировать отзыв" : "Оставить отзыв"} onClose={onClose} busy={busy} className="reviews-modal">
        <form ref={form} onSubmit={submit} noValidate className="review-form">
            ...
            <button type="submit" className="kp-button" disabled={busy}>{busy ? "Сохраняем…" : review ? "Сохранить изменения" : "Опубликовать отзыв"}</button>
        </form>
    </Modal>;
}
